...
4. зображення: multer(обробки завантаження файлів), завантаж. на свій сервер або cloudinary,

   - multer - (два основні параметри: destination (в яку директорію будуть зберігатися завантажені файли) і filename).
     🟠 npm i multer
   - Використовуємо form-data в postman. Змінюємо парсинг для form-data.
   - створюємо midlewar - multer.js, покладемо його в route/PATCH - upload.single('photo'),

Приходить запит - зберігається в тимчасовій папці temp - потім зберігається або локально(uploads) або на хмарі(cloudinary)
